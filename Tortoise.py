...
# Borrowed from TortoiseGIT package https://github.com/fyneworks/sublime-TortoiseGIT
class TortoiseBashCommand(sublime_plugin.WindowCommand):
    def run(self, paths=None, isHung=False):
        ...
        git_bash_path = settings.get('git_bash_path')

        if not os.path.isfile(git_bash_path):
            sublime.error_message(''.join(['can\'t find sh.exe (gitbash),',
                ' please config setting file', '\n   --sublime-TortoiseGIT']))
            raise

        command = 'cd %s & "%s" --login -i' % (dir, git_bash_path)
        os.system(command)
